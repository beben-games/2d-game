...
- Pausing: the upgrade menu, the build screen, and the title set `get_tree().paused`; they are CanvasLayers with `process_mode` ALWAYS (the menus on layer 10, the title on 15). Main boots into the title paused; `quiet_main()` and the smoke tool set `start_at_title` false. Anything that must run under them (input, timers) lives in those nodes. `Main.restart` and `SceneSuite.after_test` unpause. Call `Juice.reset()` before pausing so a kill freeze's time scale does not survive under the menu.
