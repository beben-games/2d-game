...
A playable single-arena vertical slice on `main` (tag `m1-candidate` at ac8f4dc, later commits add tooling and notes). Godot 4.7.2, GDScript, 108 gdUnit4 tests green, boot gate and smoke tool green.
...
Milestone 1 is a candidate awaiting the replay. The user played twice and rated every line of `docs/plans/2026-09-02-m1-feel-checklist.md` on 2026-09-04 ("Verdict, playtest 2" at the bottom of that file). Movement, impact, shake, recoil, getting hit, the chaser, and the camera were good. The five-item tuning pass from that verdict is done and committed (b932eeb through the death commit): 3x zoom and fullscreen with camera limits derived from the arena, pistol at 5 shots/s, i-frames counting real time through hitstop, solid enemy bodies with a wider hurtbox, and death holding until R. 108 tests green twice, boot gate and smoke green. The verdict was "close after tuning", so the milestone closes when the user replays and says shooting feels good.

Open issue: during playtest 1 the knight got stuck moving right. Investigation notes and a fix sketch are in `docs/plans/2026-09-04-m2-prework-notes.md` under "Open: stuck movement key". Best-supported cause: macOS discards key-ups during a title-bar window drag. It did not recur in playtest 2, and the game now runs fullscreen, which removes the title bar.

## Next steps, in order

1. The user replays against the "Replay questions" in the feel checklist (run animation at 3x, fullscreen pixels, camera scroll, 5/s cadence, i-frame length, pinning under solid bodies, death hold). Tune anything they flag with the same one-concern-per-commit loop (`tools/test.sh` twice, `tools/check_boot.sh`, `tools/smoke.sh combat`, read the PNG). Tag `m1` when they say shooting feels good.
