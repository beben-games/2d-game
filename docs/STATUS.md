...
A playable run on `main`: four single-screen rooms with an ending. Milestone 2 closed at tag `m2` on 2026-09-07; Milestone 1 closed at tag `m1`. Godot 4.7.2, GDScript, 175 gdUnit4 tests in 34 suites green, boot gate and six smoke scenarios green.
...
- Milestone 2 (candidate, tag `m2-candidate`): single-screen 28x15 rooms (`Room` scene: arena, doors, enemies, projectiles, spawner, wave runner) built from `RoomDef`s, ringed by a two-row top wall (the tileset's ledge over its brick face, with the exit door set into the band) and one-row side and bottom walls (the entry is an unpainted opening with shaded wall ends), leaving a 26x12 floor; a four-room floor (`data/floors/floor_1.tres`) with authored waves (`data/waves/room_1..4.tres`, `WaveTable` > `WaveDef` > `SpawnGroup`) driven by `WaveRunner` and the pure `WaveProgress`; the Shooter enemy (orc shaman, `ShooterBrain`) that keeps its range and fires a telegraphed 120 px/s bolt; a dash on Space or right click (`DashRules`, layer 7 so triggers still see a dashing player); a HUD with tileset hearts, room, wave, and kills; a heart pickup dropped at the room's center on clear; doors that open on clear with a real-time fade between rooms; a run summary on death ("You died") and on the last clear ("Floor cleared") that waits for R; `-- --seed=N` replays a run. The `Spawner` is a placement service now (`spawn(scene, at)`), scene tests share `tests/support/scene_suite.gd`, and the smoke tool gained `kill`, `room`, and `death` scenarios with a 30 s watchdog.
...
1. Replay the door look and the chase speed: after replay 1 the entry opening bricks up 0.4 s after you arrive (`ENTRY_SEAL_DELAY` in main.gd) and chasers run at the player's 110 px/s so the dash matters (see the M2 checklist's "Replay 2"). Earlier: the door pass landed (two-row top wall with the door set into it, dark bottom opening; see the M2 checklist's "Next pass" item 1); the clear colour is black so the openings read as dark passages; the user's playtest decides whether it closes.
