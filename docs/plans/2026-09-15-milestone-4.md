...
- Task 2: three departures from the plan's text, none in the signals or the emit sites. (1) `Audio._on_enemy_hit` reads the quiet flag duck-typed (`get_node_or_null("Health")` as a `Node`, then `get("last_hit_quiet")`) instead of `as Health`: with the class named in the autoload, `check_boot` reported three leaked ObjectDB instances and `res://scripts/settings.gd` still in use at exit (bisected by reverting each changed file, then each construct in `audio.gd`; the `Health` cast alone reproduces it, and the `UpgradeDef` parameter type does not). (2) `Audio._on_enemy_died` types `def` as `Variant`, like `RunState._on_enemy_died`: `test_run_state.gd` emits `enemy_died` with a stub whose `def` is a `RefCounted`, which a `Resource` variable refuses ("Trying to assign value of type 'RefCounted'"). (3) `test_the_menus_open_close_hover_and_pick` waits `real_seconds(0.2)` between the picker's close and the build screen's open: both closes landed inside `ui_close`'s 50 ms minimum gap, so the second was dropped by design (the wait was raised, not the code, as the plan's step 4 says for this suite).
