...
Room 2 to 4 waves reference `res://scenes/enemies/shooter.tscn`, which Task 6 creates. Until then, write the room 2 to 4 wave files with chasers only and a `# TODO(Task 6)` line in the plan, or create the files in Task 6. This plan creates `room_1.tres` and `floor_1.tres` with one room now and adds rooms 2 to 4 in Task 6, so nothing references a scene that does not exist.
