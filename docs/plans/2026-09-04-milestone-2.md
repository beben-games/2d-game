...
`scripts/room.gd`: add `@onready var wave_runner: WaveRunner = $WaveRunner`; at the end of `_ready`: `wave_runner.spawner = spawner`, `wave_runner.enemies_parent = enemies`, `wave_runner.start(def.waves)`.
