...
- Physics layers: 1 player (bit 1), 2 enemies (2), 3 player_shots (4), 4 enemy_shots (8), 5 walls (16), 6 pickups (32). Player body mask 18, enemy bodies 19, player shots 18, enemy bolts 16 (walls only; the player's hurtbox finds them), pickups mask 1.
...
Found in Task 7: dropping the enemy bit from the player's mask is not enough; the chaser's own `move_and_slide` (mask 19) depenetrates from the player each tick and shoves along with the dash. The dash therefore also sets `collision_layer = 0` (`BODY_LAYER := 1` restored in `_end_dash`). Consequence: during the 0.15 s dash the player is invisible to every Area2D that detects layer 1, which includes the door trigger (Task 4) and the heart pickup (Task 9); both rely on `body_entered` firing when the layer comes back while still overlapping. Task 10 must add a regression test: dash through an open exit door and assert `room_exit_requested` still arrives. Presses are seen by `is_action_just_pressed` on the next physics tick, so tests hold the action for two ticks; the cooldown test's third press waits 22 ticks, not 30. The smoke `move` scenario dashes at tick 52 so the puff is still visible at capture. `Input.is_action_just_pressed` in `_physics_process` is reliable in Godot 4 (it tracks the physics frame separately). The tests hold the action for two ticks so the press is seen.
...
Note the `TextureRect.scale` trick: a scaled Control still reports its unscaled size to the container, so `custom_minimum_size` reserves the scaled footprint. `RunState.kills` is incremented by RunState's own `enemy_died` handler; autoloads connect first, so the HUD reads the new count.
...
func test_dashing_through_the_open_exit_still_requests_it() -> void:
	# The dash zeroes the player's collision layer (Task 7); the trigger must still fire when the
	# layer returns while the body is inside it.
	var main := quiet_main_with_floor(tiny_floor(2))
	var player: Player = main.get_node("Player")
	var requests := [0]
	var on_exit := func() -> void: requests[0] += 1
	...
	Events.room_cleared.emit()
	await get_tree().process_frame
	player.global_position = Vector2(224, 60)
