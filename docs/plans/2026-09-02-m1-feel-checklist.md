...
- Being surrounded: enemy bodies are solid since playtest 2; the hit knockback is what gets you out of a pin until the dash arrives. Fair? (player and chaser collision_mask, hurtbox radius in scenes/player.tscn, HIT_KNOCKBACK in player.gd)
...
- Death: 0.25 s freeze, a blue burst, then the corpse holds until R (since playtest 2; the auto-restart is gone). Acceptable as the Milestone 1 contract? (Main._on_player_died in scripts/main.gd)
