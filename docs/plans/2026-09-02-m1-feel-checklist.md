...
1. **Zoom 3x and fullscreen.** `zoom = Vector2(3, 3)` on the camera in `scenes/player.tscn`; `display/window/size/mode=2` (fullscreen) in `project.godot`. At 3x the view is about 427x240 world px against a 640x368 room, so the camera now scrolls and the limits matter: move them out of `player.tscn` into `Main._ready` from `arena.bounds()` grown by one tile (the Milestone 2 should-fix, pulled forward). `tools/smoke.sh` passes `--resolution 1280x720`; add `--windowed` so smoke screenshots keep their size. If pixels shimmer at the fullscreen scale, try `display/window/stretch/scale_mode="integer"` and accept the letterbox.
...
3. **I-frames count real time across freezes.** `Juice` captures `frame_scale := Engine.time_scale` at the top of its `_physics_process` (autoloads step before scene nodes, so this is the scale Godot used for this tick's delta; the frame a hitstop starts still gets an unscaled delta, which is why dividing by the live `Engine.time_scale` would overshoot 20x). Player does `invuln_left -= delta / Juice.frame_scale`. Test: land a hit, trigger a kill freeze inside the window, assert `invuln_left` still reaches zero after 0.8 s of real ticks. Keep `INVULN_TIME` at 0.8 for the replay; shorten to 0.6 only if it still feels long once the leak is gone.
