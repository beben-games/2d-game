...
| 10 | The build screen should show the effect at the owned rank, not the per-pick line | `UpgradeDef.summary(rank)` totals the modifiers over `rank` ranks: adds sum ("+2 damage", "+2 hearts", "+2 dashes", "+2 per shot", "pierce +4"), muls compound (`roundi((pow(mul, rank) - 1) * 100)`: fire rate +25%, +56%, +95%), `spread_degrees` is silent, a flag stat (bounce, homing, burn, stun, chill) or no modifiers returns the description. `PHRASES` on `UpgradeDef` names the stats. `BuildScreen._rebuild` prints `summary(build.rank_of(id))`; the card offer keeps the per-pick description. Tests: every real card at the ranks above, the build screen at two ranks, the glyph test covers every summary at every rank. | `scripts/defs/upgrade_def.gd`; `scripts/ui/build_screen.gd`; `tests/test_upgrade_def.gd`, `tests/test_build_screen_scene.gd`, `tests/test_assets.gd` | |
