...
| 9 | The Heal card repeats itself: "Restore one heart" then "One heart" | `UpgradeMenu.rank_line` returns "" for HEAL and `_card` adds no rank label when the line is empty (the heal card is icon, name, effect). `test_rank_line_per_kind` expects ""; a new test checks the heal card holds two labels; the fit test still runs every card. | `scripts/ui/upgrade_menu.gd`; `tests/test_upgrade_menu_scene.gd` | Done in 80f73ec |
| 10 | The build screen should show the effect at the owned rank, not the per-pick line | `UpgradeDef.summary(rank)` totals the modifiers over `rank` ranks: adds sum ("+2 damage", "+2 hearts", "+2 dashes", "+2 per shot", "pierce +4"), muls compound (`roundi((pow(mul, rank) - 1) * 100)`: fire rate +25%, +56%, +95%), `spread_degrees` is silent, a flag stat (bounce, homing, burn, stun, chill) or no modifiers returns the description. `PHRASES` on `UpgradeDef` names the stats. `BuildScreen._rebuild` prints `summary(build.rank_of(id))`; the card offer keeps the per-pick description. Tests: every real card at the ranks above, the build screen at two ranks, the glyph test covers every summary at every rank. | `scripts/defs/upgrade_def.gd`; `scripts/ui/build_screen.gd`; `tests/test_upgrade_def.gd`, `tests/test_build_screen_scene.gd`, `tests/test_assets.gd` | Done in 651527d |
| 11 | Rank bonuses should read 25 / 50 / 75, not compound ("they shouldn't stack cumulatively, that's confusing to people") | Ranks stack additively: `Modifier.apply_ranks(value, rank)` applies a modifier once at the owned rank as `(value + add * rank) * (1 + (mul - 1) * rank)`, so fire rate at rank 1, 2, 3 is +25%, +50%, +75% of the base; `Build.resolve` and `_fold_player` drop the per-rank loop and round int stats once after the fold (`roundi`, half away from zero); `UpgradeDef.summary` prints `roundi((mul - 1) * 100 * rank)`. The card offer's per-pick description ("+25% fire rate") is unchanged. Tests: rank 2 fire rate is 4 * 1.5 = 6, the int-stat mul tests round once, three ranks in the player scene give 5 * 1.75, summaries +50% / +75%, the build screen row "+50% fire rate". | `scripts/defs/modifier.gd`; `scripts/build.gd`; `scripts/defs/upgrade_def.gd`; `tests/test_build.gd`, `tests/test_upgrade_def.gd`, `tests/test_player_build_scene.gd`, `tests/test_build_screen_scene.gd`; design doc "Data" | Done in 1613920 |
| 12 | The switch cards should say what makes each weapon specific, and how many upgrades you will re-pick | Descriptions name the weapon's feel and its upgrade path: handgun "Fast, accurate bullets. Path: rapid fire and tracing."; crossbow "Slow, heavy bolts pierce two. Path: fire, shock, chill." (the longer wordings first tried, "Its path: rapid fire and tracing bullets" / "Its path: fire, shock, and chill", wrapped to four and five lines of the 32 px body font in the 264 px column and ran the rank line under the frame; three lines is the most the column takes over a two-line swap count). `rank_line` for SWITCH always states the count: "Swap now, nothing to re-pick", "Swap and re-pick 1 upgrade", "Swap and re-pick n upgrades". Tests: `test_rank_line_per_kind` pins the three strings; the fit test and the glyph test cover the new text. | `data/upgrades/switch_handgun.tres`, `switch_crossbow.tres`; `scripts/ui/upgrade_menu.gd`; `tests/test_upgrade_menu_scene.gd` | Done in 1f5eeea |
| 13 | Flaming, shock, and chill bolts should look different in flight | `Projectile._ready` dresses a shot carrying burn, stun, or chill: the `Bolt` sprite takes the matching `StatusEffects` tint as its modulate (a bullet, having no sprite, tints its `core_color`/`glow_color`), in the order `StatusEffects._tint` uses (stun over burn over chill), and a `CPUParticles2D` child `Trail` emits continuously behind the shot (`local_coords` off, direction LEFT in the shot's frame, spread 20, speed 10 to 30, lifetime 0.35, `Fx.fade_scale()` / `Fx.fade_ramp()` shared from `fx.gd`, which gains `class_name Fx`): burn adds an upward gravity so the embers rise, chill none, stun a sparser wider sparkle. Amount 24 at scale 1.5 to 2.5 (12 for stun): the plan's 12 at 1 to 2 read as three or four scattered specks at 3x zoom; 24 reads as a line of embers. Constants at the top of `projectile.gd`; the trail frees with the shot. Tests: a flaming bolt has an emitting `Trail` in the burn tint with the burn gravity and a tinted sprite; chill takes the chill tint; burn plus chill gives burn, stun plus burn gives stun; a plain bolt has no trail and a white sprite; a stunning bullet trails too and tints its core. Screenshot: `reports/flaming_shot.gd` (throwaway, gitignored) fires a flaming crossbow bolt on the smoke floor. | `scripts/projectile.gd`; `scripts/fx.gd`; `tests/test_projectile.gd` | Done in b6a169b |
