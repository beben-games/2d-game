...
**The flow.** The title's Play starts the first run straight in the arena while `flags.returned` is false (a yielded first run or an Esc on the first gate screen lands there again; the grounds appear only after the first gate screen is passed).
...
- Task 5: the piles still on the floor at a thumb up are swept into the run's coins before the banking (a coordinator decision; "piles stay until picked up or the run ends" reads "or are swept at a thumbs up"); a yield counts a run as well as a fall; the gate screen pauses the tree and R on it restarts; the `win` and `lose` stings went (the verdict's sounds are the run's sting); the thumb sits over the box with its top on the arena's edge (a tile above it is off screen). The rest is as designed.
- Task 6: the gate screen's continue leads to the grounds without a restart (the run closed at the verdict); a restart or a quit from the grounds yields nothing; the rack hangs a knight sword, an axe, and a bow; the armoury shows the handgun (every run starts with it) and two greyed slots; the rows' clicks reach greyed rows too (the refusal has its sound); R and Restart do nothing in the grounds (the gate is the only way in), and nothing fires there. The rest is as designed.
- Task 4: a per-id stat files a source with no id under "unknown" (the signals carry "" as written); `dashes_through_danger` counts daring kills (the act is scored per kill since Task 2); `flags.returned` is a bool, the other flags counts; the tests set the scratch path in `SceneSuite.before_test` as well as resetting in `after_test`. The rest is as designed ("`hits_by`" in the profile section reads `hits_taken`).
- Final review (2026-09-25): the first-run rule is `flags.returned`, not `flags.runs == 0` (the build's rule; the paragraph above now says so). `tests/test_run_state.gd` read the live profile and would have failed once a real save held a training rank: it points the profile at the scratch now.
- Task 7: none at the design level. The docs (`docs/TESTERS.md` rewritten under "show, don't tell": controls, two facts, questions, and a "For testers only" note for the cheats; `docs/STATUS.md`; `CLAUDE.md`; `docs/plans/2026-09-25-m5-feel-checklist.md` with a knob per row), the version `0.5.0-rc1`, the pre-release `v0.5.0-rc1`, and the tag `m5-candidate`. The milestone closes on the user's playtest verdict; the training lines' names and prices, the code-drawn thumb, and the verdict's rule stay the placeholders this design named.
