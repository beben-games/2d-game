...
## The shield (playtest 1)

Added 2026-09-23 from the user's first playtest of the candidate: once the handgun has tracing and piercing it is overpowered, so enemies need a side that projectiles cannot hit. The user chose a shield attribute any enemy def can carry over a new enemy type.

`EnemyDef` gains `shield: bool` (default false), `shield_arc_degrees: float` (120 since playtest 1's note 5; 180, the front half, was the first cut and too big), and `shield_turn_degrees: float` (60 per second since the same note: how fast the shield's facing follows the enemy's movement; 180 was the first cut, too few handgun shots per dash, then 90, which turned around too quickly). An enemy with a shield keeps a `facing` vector: it starts toward the player at spawn and each tick turns toward the direction it is moving (the wish, or `to_target` when standing) at no more than the turn rate, so a chaser's shield points at the player while it chases but lags when the player dashes past, and a flanking shot lands; a stun holds the facing and a chill halves the turn. A player shot arriving from inside the arc around `facing` is blocked unless its `pierce` is at least `Enemy.SHIELD_PIERCE` (3): it despawns on the shield with steel sparks and a clink (`Events.shot_blocked`, the `shot_shield` sound); anything else hits as today. Enemy bolts are unaffected (they never hit enemies). Homing shots home as today and are blocked like any other. The shield draws as an arc of pale steel in front of the sprite (`ShieldArc`, a `Node2D` child that draws a thick arc across the shield's angle at a radius just outside the body, rotated to `facing`), so the player reads the covered side; the user may supply a shield sprite later to replace the arc. A corpse blocks nothing. The pure rules are `ShieldRules.blocks` and `ShieldRules.turn`.

Which builds pass: the handgun's Piercing bullets is one rank of +1 on a base of 0, so a handgun shot never passes a shield; the crossbow's base pierce is 2 and Deep pierce adds 2 a rank over two ranks, so a crossbow with at least one Deep pierce rank (pierce 4, or 6) passes. The shipped carrier is `chaser_shield` (the chaser's numbers and art, score 15), replacing plain chasers from room 4 on with the totals of the balance pass unchanged: room 4 waves 0 / 2 / 3, room 5 2 / 3 / 3, room 6 3 / 3 / 4, room 7 4 / 4 / 5. Rooms 1 to 3 and the boss are untouched.
...
- Task 10 review: Quit game saves the volumes before quitting; the tester notes name Godot's per-user folder; the checklist gains "Task 10 notes" rows; the HUD hides under the title; a 40 px spacer (not 72, which overflowed the widest build) keeps the weapon name clear of the frame's ornament; junk in the seed field is tinted; an immortal player despawns the bolts that land; `Cheats.parse` returns the table's row and `start_run` copies; tests for R on the summary and Esc inside the picker beat (`Main.restart()` hides the summary); `CLAUDE.md`, `tools/run.sh`, and `docs/STATUS.md` drift.
- Task 11: the shield's facing is seeded on the enemy's first physics tick, not in `_ready` (the body is placed after `add_child`); the test suite's `STAND_INS` map lets `shot_shield` play the `shot_wall` file; at 180 degrees a second a full reversal takes one second (side-on at half a second); a corpse hides its arc; the arc carries a darker 1 px rim.
- Task 11 review: the arc fades in with the sprite (a sibling, not a child), the facing is seeded on demand from `blocks_shot` too, the turn rate is 90 (180 was the first cut) and a chill halves it.
