...
| 4 | The first boss is still far too weak (two wins in about three minutes each) | `max_hp` 300 to 600, `telegraph_time` 0.6 to 0.5, `recover_time` 0.8 to 0.55, `charge_speed` 320 to 380; stage two keeps its own numbers; tune again on the next run | `data/enemies/boss.tres` | |
...
| 14 | The bonuses in the top right corner should reset between series | Bug: the HUD build strip and the counter refresh on `run_started` (a run from the gate has no scene reload) | `scripts/ui/hud.gd` | |
