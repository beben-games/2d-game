...
|---|---|---|---|---|
| 1 | Need to grab coins from further away (a pull) | Piles inside `PULL_RADIUS` (96 px) drift to the player at `PULL_SPEED`, accelerating; `Reach` ranks widen it | `scripts/coin_pile.gd` | Done in 5ecc413 |
...
| 6 | The thumb is ugly and disproportionate | A 24x24 placeholder drawn with a proper fist (thumb, knuckles, a cuff) at 2x, centred over the box; M8's art replaces it | `scripts/thumb_sign.gd` | Done in 067c602 |
...
| 16 | I don't understand the bars under the life bar | An icon at the left of each: a boot for the dash pips, a small crowd for the favour meter, from the Raven sheet or drawn in code until M8 | `scripts/ui/hud.gd` | Done in 224dcab |
