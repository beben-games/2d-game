...
The knobs, in order: `shield_turn_degrees` in `data/enemies/chaser_shield.tres` (now 90; 180 was the first cut, too few shots per dash), then `shield_arc_degrees` 180 to 120 (a narrower front), then the shielded counts in `data/waves/room_7.tres` from 4 / 4 / 5 to 3 / 3 / 4 (the totals unchanged: plain chasers take the difference).
