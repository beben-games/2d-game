...
- Modify: `scripts/enemy.gd` (the bolt scene is loaded on the first shot, not preloaded; see Deviations)
...
`scripts/enemy.gd`: `const ENEMY_BOLT := preload("res://scenes/enemies/enemy_bolt.tscn")` becomes `const ENEMY_BOLT_PATH := "res://scenes/enemies/enemy_bolt.tscn"` and `_fire_bolt` instantiates `load(ENEMY_BOLT_PATH)`, with a comment naming the reason: `projectile.gd` now names `Enemy` (`_nearest_enemy`), `enemy.gd` names the bolt scene, and the bolt scene names `projectile.gd`; a preload closes that cycle while `projectile.gd` is still loading, and Godot then caches the bolt scene without its script ("[ext_resource] referenced non-existent resource"), so shooters would fire plain `Area2D`s. `load` at fire time is a cache hit after the first shot.

**Step 4: Run the tests to verify they pass**

Run: `tools/test.sh` (the whole set: chaser, shooter, floor, and juice suites all fire shots).
Expected: all green. The homing code is in place but untested until Task 8; `test_shooter_scene` still sees bolts hit the player through the hurtbox. Then `tools/check_boot.sh`: the load cycle above shows there (and in the projectile suite run alone) but not in the full suite, where another suite loads `enemy.gd` first.
