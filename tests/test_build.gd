...
## Every run starts from the same bases: the training buys nothing a card gives.
func test_a_fresh_build_folds_the_cards_over_the_base_constants() -> void:
	var build := Build.new()
	assert_int(build.max_hp(_catalog())).is_equal(Build.BASE_MAX_HP)
	assert_int(build.dash_charges(_catalog())).is_equal(Build.BASE_DASH_CHARGES)
	build.add_rank(_catalog()["heart"])
	build.add_rank(_catalog()["dash"])
	assert_int(build.max_hp(_catalog())).is_equal(Build.BASE_MAX_HP + 2)
	assert_int(build.dash_charges(_catalog())).is_equal(Build.BASE_DASH_CHARGES + 1)
