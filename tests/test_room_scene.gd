...
func test_entry_position_is_one_tile_inside_the_entry_door_or_the_center() -> void:
	...
	assert_vector(first.entry_position()).is_equal(Vector2(224, 128))
	...
	assert_vector(from_top.entry_position()).is_equal(Vector2(224, 40))


func test_closed_door_blocks_the_player() -> void:
	var main := quiet_main()
	var player: Player = main.get_node("Player")
	player.global_position = Vector2(224, 56)  # under the top door gap
	player.aim_override = player.global_position
	Input.action_press("move_up")
	await ticks(40)
	Input.action_release("move_up")
	# The top band ends at y 32 and the body is r 6: held at about y 38, never into the gap.
...
func test_open_door_lets_the_player_through_and_requests_exit() -> void:
	...
	room.open_exit()
	player.global_position = Vector2(224, 56)
	player.aim_override = player.global_position
	Input.action_press("move_up")
	await ticks(40)
	Input.action_release("move_up")
	Events.room_exit_requested.disconnect(on_exit)
	assert_float(player.global_position.y).is_less(32.0)
	assert_int(requests[0]).is_greater_equal(1)


func test_dashing_through_the_open_exit_requests_it() -> void:
	# A short dash from right under the gap crosses the 32 px trigger inside the dash, so the
	...
	player.global_position = Vector2(224, 46)
